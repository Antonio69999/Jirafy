...
export function EditStatusModal({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updates: any = {};
    if (key !== status?.key) updates.key = key.toUpperCase();
    if (name !== status?.name) updates.name = name;
    if (category !== status?.category) updates.category = category;
      ...
      >
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold">
            {t("workflow.status.edit") || "Modifier le statut"}
          </DialogTitle>
          <DialogDescription className="text-sm opacity-80">
            Modifier les informations du statut "{status.name}"
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-4">
          ...
          <div className="flex justify-end gap-2 mt-6">
            ...
            >
              {t("workflow.status.update") || "Modifier"}
            </Button>
          </div>
        </form>
      </DialogContent>
